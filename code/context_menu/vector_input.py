...
class VectorInput():
    ...
    def __create_text_surfaces(self) -> list:
        ...
        for option in ['Power: 0', 'Reset', 'Save']:
    ...
    def __update_input_surface(self, mouse_pos: vector) -> None:
            ...
            self.power = mouse_pos.distance_to(self.input_rect.center)
            angle = math.atan2(mouse_pos.y - self.input_rect.centery, mouse_pos.x - self.input_rect.centerx)
            decrease_acc = 25
            vector_acceleration = vector(math.cos(angle) * self.power / decrease_acc, math.sin(angle) * self.power / decrease_acc)
            self.power_vector = vector_acceleration

            self.text_surfaces[0] = (self.font.render(f'Power {round(self.power, 1)}', True, (0, 0, 0)).convert_alpha(), f'Power {round(self.power, 1)}')
            self.__draw_text_surface()
